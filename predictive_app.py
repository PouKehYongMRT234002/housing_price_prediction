...
st.set_page_config(page_title="Price Prediction App with Monitoring",
                   layout="centered")

st.title("Price Prediction App with Live Monitoring")

@st.cache_resource
def load_models():
    old_model = joblib.load("model_v1.pkl")  # trained on ["area"]
    new_model = joblib.load("model_v2.pkl")  # trained on everything field except area
...
area = st.sidebar.number_input("Area (sq ft)", 500, 10000, 3000)
...
if st.button("Run Prediction"):
    ...
    st.session_state["input_summary"] = f"area={area}, bedrooms={bedrooms}, bathrooms={bathrooms}, stories={stories}, mainroad={mainroad}, guestroom={guestroom}, basement={basement}, hotwaterheating={hotwaterheating}, airconditioning={airconditioning}, airconditioning, parking={parking}, prearea={prefarea}, furnishingstatus={furnishingstatus}"
...
if st.session_state["pred_ready"]:
    ...
    st.write(f"New Model (v2 - all total fields): **${st.session_state['new_pred']:,.2f}**")
